...
class CustomTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, path_embed, vel_embed, path_vocab, vel_vocab, traj_vocab, traj_mask,
        ...
        # hoisted: feature-map derived tensors are identical across decoder layers; compute once
        if "deform_value" not in camera_feature:
            camera_feature["img_value"] = camera_feature["feature_maps"][-1].permute(0, 1, 3, 4, 2).flatten(1, 3)
            camera_feature["deform_value"] = deformable_format(camera_feature["feature_maps"])
        img_value = camera_feature["img_value"]
        deform_value = camera_feature["deform_value"]
        ...
        if self.decoder_idx == self._config.decoder_num_layers - 1:
            ...
            output["traj_scores"] = traj_scores
            for _m in metric_logit:
                output[f"metric_{_m}"] = metric_logit[_m]
