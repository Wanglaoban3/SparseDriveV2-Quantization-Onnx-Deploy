...
def feature_maps_format(feature_maps, inverse=False):
    ...
    scale_start_index = torch.cat(
        [scale_start_index.new_zeros(1), scale_start_index[:-1]]
    )
...
def deformable_format(
    ...
    if spatial_shapes is None:
        ...
        for lvl, feat in enumerate(feature_maps):
            # create the shape tensor on-device directly; a CPU constant + .to(device)
            # breaks ONNX constant folding with mixed-device cat inputs
            spatial_shape = torch.tensor(feat.shape[-2:], dtype=torch.int64, device=feat.device)
