...
from navsim.navsim_v1.common.dataclasses import AgentInput, Scene, SceneFilter, SensorConfig
...
class MetricCacheLoader:
    ...
    def _load_metric_cache_paths(self, cache_path: Path) -> Dict[str, Path]:
        ...
        # normalize to forward slashes so token extraction works cross-platform
        cache_paths = [p.replace("\\", "/") for p in cache_paths]
